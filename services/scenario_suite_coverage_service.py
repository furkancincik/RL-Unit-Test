...
class ScenarioSuiteCoverageResult:
    """
    Birden fazla test senaryosunun birlikte çalıştırılmasıyla elde edilen
    coverage sonucunu temsil eder.
    ...
    def success(self) -> bool:
        """Pytest ve coverage işleminin başarılı olup olmadığını belirtir."""
        return self.coverage.success

    @property
    def has_full_coverage(self) -> bool:
        """Test paketinin tam coverage sağlayıp sağlamadığını belirtir."""
        return self.coverage.has_full_coverage


class ScenarioSuiteCoverageService:
    """
    Birden fazla Scenario nesnesini tek bir pytest dosyasına dönüştürür
    ve kümülatif coverage sonucunu ölçer.

    Bu servis RL algoritmasını, Action nesnesini veya QTable yapısını
    bilmez. Yalnızca kendisine verilen senaryo grubunu çalıştırılabilir
    test paketine dönüştürür.
    """
    ...
    def _create_test_file_name(
        ...
        return f"test_{function_name}_scenario_suite.py"
